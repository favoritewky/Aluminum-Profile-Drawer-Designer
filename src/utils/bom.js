// ─── BOM 整理特殊规则 ──────────────────────────────────────────────────────────
//
// 1. 合并规则：同 name + spec + scope 的条目自动累加 qty，跨 scope 的同名条目保持独立。
//
// 2. 连接件类型影响型材截取长度：
//    - 三通角码（tee）：前/后横梁和侧向横梁均缩短 dpW（两端各让位），
//      尺寸 = 净内宽/深 − 2×dpW。
//    - L角码 / 槽用角码 / T螺母：前/后横梁取全净内宽；侧向横梁缩短 2×dpW
//      （夹在前后横梁之间），尺寸 = 净内深 − 2×dpW。
//
// 3. 螺钉计算：每个角码需 3 颗螺钉；每个抽屉底部 4 角 = 12 颗。
//
// 4. 侧装滑轨安装横梁：每个铝制抽屉在柜体侧面各加一根横梁，
//    条目以"侧面滑轨安装横梁（抽屉N）"命名，确保跨抽屉不合并，
//    note 中记录中心距柜顶距离（mm）供现场定位。
//
// 5. 抽屉底板 / 后板 / 侧板均向内缩进 dpW（嵌入型材内侧），
//    尺寸 = 净内宽/深 − 2×dpW。
//
// 6. 面板（面板）尺寸由 calcFacePanelSize 计算，支持四向扩展模式
//    （none / half / full），half = 延伸 profileW/2，full = 延伸整个 profileW。
//
// 7. 木质抽屉箱体结构与铝框不同：
//    侧板取全净内深；前/后板宽度 = 净内宽 − 2×侧板厚；底板同理缩进侧板厚。
//
// ─────────────────────────────────────────────────────────────────────────────
